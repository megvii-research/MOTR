...
def detmotdet_evaluate(model, data_loader, device, iou_thres=0.5, print_interval=10):
    ...
    # Print mAP per class
    # print('%11s' * 5 % ('Cur Image', 'Total', 'P', 'R', 'mAP') + ',' + '%11s' * 5 % ('Next Image', 'Total', 'P', 'R', 'mAP'))

    # print('AP: %-.4f\n\n' % (cur_AP_accum[0] / (cur_AP_accum_count[0] + 1E-16))+ ',' + 'AP: %-.4f\n\n' % (next_AP_accum[0] / (next_AP_accum_count[0] + 1E-16)))
