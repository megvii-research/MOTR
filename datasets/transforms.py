...
def random_shift(image, target, region, sizes):
    ...
    if "masks" in target:
        # FIXME should we update the area here if there are no boxes?
        # target['masks'] = target['masks'][:, i:i + h, j:j + w]
        target['masks'] = interpolate(target['masks'][:, i:i + h, j:j + w][:, None].float(), sizes, mode="nearest")[:, 0] > 0.5
        fields.append("masks")
...
class Mosaic(object):
    def __init__(self, common_size=(1280, 720), min_offset=0.2, mode='resize'):
        self.common_size = common_size
        self.min_offset = min_offset
        self.min_w, self.min_h = self.common_size
        self.cut_x = random.randint(int(self.min_h * self.min_offset), int(self.min_h * (1 - self.min_offset)))
        self.cut_y = random.randint(int(self.min_w * self.min_offset), int(self.min_w * (1 - self.min_offset)))
        self.mode = mode
        assert self.mode in ['resize', 'crop']

    def __call__(self, imgs, targets):
        ret_imgs = []
        ret_targets = []
        min_w, min_h = self.min_w, self.min_h
        cut_x, cut_y = self.cut_x, self.cut_y
        for i, (img, target) in enumerate(zip(imgs, targets)):
            img, target = resize(img, target, self.common_size)
            if i == 0:
                crop_size = (cut_x, cut_y)
                ret_target = copy.deepcopy(target)
            elif i == 1:
                crop_size = (cut_x, min_w - cut_y)
            elif i == 2:
                crop_size = (min_h - cut_x, cut_y)
            else:
                crop_size = (min_h - cut_x, min_w - cut_y)
            if self.mode == 'crop':
                crop_func = RandomCrop(size=crop_size)
                img, target = crop_func(img=img, target=target)
            elif self.mode == 'resize':
                img, target = resize(img, target, crop_size[::-1])
            else:
                assert self.mode in ['resize', 'crop'] 
            ret_imgs.append(img)
            ret_targets.append(target)

        def offset_bbox(target, x_offset, y_offset):
            target['boxes'][:,0::2] += x_offset
            target['boxes'][:,1::2] += y_offset
            return target

        def offset_mask(target, padding):
            target['masks'] = torch.nn.functional.pad(target['masks'], padding).to(torch.uint8)
            return target

        def offset_obj_id(target, offset):
            target['obj_ids'] += offset
            return target

        # offset boxes
        ret_targets[1] = offset_bbox(ret_targets[1], cut_y, 0)
        ret_targets[2] = offset_bbox(ret_targets[2], 0, cut_x)
        ret_targets[3] = offset_bbox(ret_targets[3], cut_x, cut_y)

        # padding masks
        ret_targets[0] = offset_mask(ret_targets[0], (0, min_w - cut_y, 0, min_h - cut_x)) # (l,r,t,b)
        ret_targets[1] = offset_mask(ret_targets[1], (cut_y, 0, 0, min_h - cut_x)) 
        ret_targets[2] = offset_mask(ret_targets[2], (0, min_w - cut_y, cut_x, 0))
        ret_targets[3] = offset_mask(ret_targets[3], (cut_y, 0, cut_x, 0))

        # re-order to make sure obj_ids unique
        ret_targets[1] = offset_obj_id(ret_targets[1], 100)
        ret_targets[2] = offset_obj_id(ret_targets[2], 200)
        ret_targets[3] = offset_obj_id(ret_targets[3], 300)

        def concat(inputs):
            # collect all the data into result
            top = np.concatenate([inputs[0], inputs[1]], axis=1)
            bottom = np.concatenate([inputs[2], inputs[3]], axis=1)
            ret = np.concatenate([top, bottom], axis=0)
            return ret

        # collect all the data into result
        ret_imgs = [np.array(v) for v in ret_imgs]
        ret_img = Image.fromarray(concat(ret_imgs))
        for key in ['boxes', 'masks', 'labels', 'obj_ids', 'area', 'iscrowd']:
            ret_target[key] = torch.cat([item[key] for item in ret_targets], dim=0)
        ret_target['size'] = torch.tensor([min_h, min_w])
        return ret_img, ret_target


class MotMosaic(object):
    def __init__(self, common_size=(1280, 720), min_offset=0.2):
        self.mosaic = Mosaic(common_size=common_size, min_offset=min_offset)

    def __call__(self, imgs, targets):
        '''
        imgs: List[List], [seqs0, seqs1, seqs2, seqs3]
        targets: List[List], [targets0, targets1, targets2, targets3]
        '''
        ret_imgs = []
        ret_targets = []
        for i in range(len(imgs[0])):
            img, target = self.mosaic(imgs=[item[i] for item in imgs], targets=[item[i] for item in targets])
            ret_imgs.append(img)
            ret_targets.append(target)
...
# class FixedMotRandomCrop(RandomCrop):
#     def __call__(self, imgs: list, targets: list):
#         ret_imgs = []
#         ret_targets = []
#         region = T.RandomCrop.get_params(imgs[0], self.size)
...
class MotRandomShift(object):
    ...
    def __call__(self, imgs: list, targets: list):
        ...
        # debug for visual 
        # its = random.randint(1,1000000)
        # for j, (img, target) in enumerate(zip(ret_imgs, ret_targets)):
        #     boxes = target['boxes']
        #     obj_ids = target['obj_ids']
        #     draw = ImageDraw.Draw(img)
        #     for i,label in enumerate(boxes):
        #         draw.rectangle(label.tolist(), outline=tuple(np.random.randint(0, 255, size=[3])))
        #     img.save(os.path.join('/data/dongbin/projects/deformable_motr/exps/debug',f'{its}_{j}.jpg'))
        return ret_imgs, ret_targets
...
class ShiftLabeledImage(object):
    def __init__(self, bs=1, padding=75):
        self.bs = bs
        self.padding = padding
    ...
    def __call__(self, imgs: list, targets: list):
        ...
        for i in range(n_frames):
            if targets[i]['shift_label'] == 1:
                ...
                xmin = max(0, -xshift[0])
                xmax = min(w, w - xshift[0])
                prev_img = imgs[i].copy()
                prev_target = copy.deepcopy(targets[i])
                region = (int(ymin), int(xmin), int(ymax - ymin), int(xmax - xmin))
                img_i, target_i = random_shift(prev_img, prev_target, region, (h, w))
            else:
                img_i = imgs[i]
...
class MotNormalize(Normalize):
    def __call__(self, imgs, targets=None, vis=False):
        if vis:
            # visual for debug
            its = random.randint(1,1000000)
            for j, (img, target) in enumerate(zip(imgs, targets)):
                ...
                for i,label in enumerate(boxes):
                    draw.rectangle(label.tolist(), outline=tuple(np.random.randint(0, 255, size=[3])))
                img.save(os.path.join('/data/dongbin/projects/motr_bdd100k/exps/debug',f'{its}_{j}.jpg'))
