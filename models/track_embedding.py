...
class AttentionMerger(TrackEmbeddingBase):
    def __init__(self, args, dim_in, hidden_dim, dim_out):
    ...
    def _build_layers(self, args, dim_in, hidden_dim, dim_out):
        ...
        if args.update_query_pos:
            self.linear3 = nn.Linear(dim_in, hidden_dim)
            self.linear4 = nn.Linear(hidden_dim, dim_in)
    ...
    def _add_fp_tracks(self, track_instances: Instances, active_track_instances: Instances) -> Instances:
        ...
        inactive_idxes = (track_instances.scores < (self.score_thresh - 0.2)) & (track_instances.obj_idxes < 0)
        inactive_tracks = track_instances[inactive_idxes]
        num_inactive_tracks = len(inactive_tracks)
        num_add_inactive_tracks = int(self.fp_ratio * num_active_tracks)

        if num_inactive_tracks > num_add_inactive_tracks > 0:
            fp_idx = torch.randperm(num_inactive_tracks)[:num_add_inactive_tracks]
            fp_track_instances = inactive_tracks[fp_idx]
    ...
    def _select_active_tracks(self, data: dict) -> Instances:
        ...
        if self.training:
            active_idxes = (track_instances.scores >= self.score_thresh) & ((track_instances.iou > 0.3) | (track_instances.obj_idxes < 0))
            active_track_instances = track_instances[active_idxes]
            # set -2 instead of -1 to ensure that these tracks will not be selected in matching.
            active_track_instances.obj_idxes[(active_track_instances.obj_idxes < 0)] = -2
    ...
    def _update_track_embedding(self, track_instances: Instances) -> Instances:
        ...
        if self.update_query_pos:
            query_pos2 = self.linear4(self.dropout3(self.activation(self.linear3(tgt))))
            query_pos = query_pos + self.dropout4(query_pos2)
            query_pos = self.norm3(query_pos)
        ...
        track_instances.query_pos[:, dim//2:] = tgt
...
class AttentionMergerV2(AttentionMerger):
    def _build_layers(self, args, dim_in, hidden_dim, dim_out):
...
class AttentionMergerV3(AttentionMergerV2):
    def _select_active_tracks(self, data: dict) -> Instances:
        track_instances: Instances = data['track_instances']
        if self.training:
    ...
    def _add_fp_tracks(self, track_instances: Instances, active_track_instances: Instances) -> Instances:
        ...
        if len(inactive_instances) > 0 and len(active_track_instances) > 0:

            num_fp = int(self.fp_ratio * num_active_tracks)
            if num_fp >= len(inactive_instances):
                ...
                active_boxes = Boxes(box_ops.box_cxcywh_to_xyxy(active_track_instances.pred_boxes))

                ious = pairwise_iou(inactive_boxes, active_boxes)
                max_iou = ious.max(dim=-1).values

                fp_idxes = torch.topk(max_iou, num_fp,).indices
                fp_track_instances = inactive_instances[fp_idxes]
...
class AttentionMergerV4(AttentionMergerV3):
    def _add_fp_tracks(self, track_instances: Instances, active_track_instances: Instances) -> Instances:
        inactive_instances = track_instances[track_instances.obj_idxes < 0]
...
class AttentionMergerV4RefBoxes(AttentionMergerV3):
    def _add_fp_tracks(self, track_instances: Instances, active_track_instances: Instances) -> Instances:
        inactive_instances = track_instances[track_instances.obj_idxes < 0]
...
class AttentionMergerV5(AttentionMergerV4):
    def _add_fp_tracks(self, track_instances: Instances, active_track_instances: Instances) -> Instances:
        fps = track_instances[track_instances.obj_idxes < 0]
        high_score_fps = fps[fps.scores >= 0.8]
        low_score_fp = fps[fps.scores < 0.8]
        merged_tracks = super()._add_fp_tracks(low_score_fp, active_track_instances)
        if len(high_score_fps) == 0:
            return merged_tracks
        high_score_fps.obj_idxes[:] = -2  # set obj_id of FPs to -2 to ensure they will not be selected in matching.
        merged_tracks = Instances.cat([merged_tracks, high_score_fps])
        return merged_tracks

    def _select_active_tracks(self, data: dict) -> Instances:
        track_instances: Instances = data['track_instances']
        if self.training:
            # active_idxes = (track_instances.obj_idxes >= 0) & (track_instances.iou > 0.3)
            active_idxes = (track_instances.obj_idxes >= 0) & (track_instances.iou > 0.5)
            active_track_instances = track_instances[active_idxes]
            # set -2 instead of -1 to ensure that these tracks will not be selected in matching.
            active_track_instances = self._random_drop_tracks(active_track_instances)
...
class AttentionMergerV6(AttentionMerger):
    def _build_layers(self, args, dim_in, hidden_dim, dim_out):
        dropout = args.merger_dropout

        print(hidden_dim, dim_in, flush=True)
    ...
    def _update_track_embedding(self, track_instances: Instances) -> Instances:
        ...
        query_pos = track_instances.query_pos[:, :dim // 2]
        query_feat = track_instances.query_pos[:, dim//2:]
        tgt = query_pos + out_embed

        updated_query_pos = self.dropout(self.activation(self.linear1(tgt)))
        update_query_feat = self.dropout(self.activation(self.linear2(tgt)))

        if self.update_query_pos:
            track_instances.query_pos[:, :dim // 2] = updated_query_pos
        track_instances.query_pos[:, dim//2:] = update_query_feat
...
def build(args, layer_name, dim_in, hidden_dim, dim_out):
    embedding_layers = {
        'AttentionMerger': AttentionMerger,
        'AttentionMergerV2': AttentionMergerV2,
        'AttentionMergerV3': AttentionMergerV3,
        'AttentionMergerV4': AttentionMergerV4,
        'AttentionMergerV5': AttentionMergerV5,
        'AttentionMergerV4RefBoxes': AttentionMergerV4RefBoxes,
        'AttentionMergerV6': AttentionMergerV6,
    }
    assert layer_name in embedding_layers, 'invalid track embedding layer: {}'.format(layer_name)
